...
<?php
    ...
    $sql = "SELECT * FROM tb_santri WHERE NIS = '$NIS'";
...
?>
  ...
  <div class="container container-form">
    <div class="row">
      <div class="span12">
      <legend>Ubah Data Santri</legend>
        <form method="POST" action="perbarui-data-santri.php">
          ...
          <div class="form-group">
            ...
            <input type="text" name="NamaSantri" value="<?php echo $data2['NamaSantri']; ?>" class="form-control" id="NamaSantri" required></input>
            </div>
          </div>
          <div class="form-group">
            <div class="input-group col-md-5">
          ...
          <div class="form-group">
            <div class="input-group col-md-5">
            <label>Alamat</label>
            <textarea name="AlamatSantri" class="form-control" required><?php echo $data2['AlamatSantri']; ?></textarea>
            </div>
          </div>
          <div class="form-group">
            <div class="input-group col-md-5">
            <label>Kamar</label>
            <select name="KodeKmr" class="form-control">
            <option value="<?php echo $data2['KodeKmr']; ?>"><?php echo $data2['KodeKmr']; ?></option>
                <?php
                  include '../lib/koneksi.php';
                  $sql = "SELECT * FROM tb_kamar";
                  $result = mysqli_query($conn, $sql);
                  while($data=mysqli_fetch_array($result)){ ?>
                    <option value="<?php echo $data['KodeKmr']; ?>" <?php if($data['KodeKmr']==$data2['KodeKmr']){ echo "Selected"; }else{ echo ""; } ?>><?php echo $data['NamaKmr']; ?></option>
                  <?php } ?>
            </select>
            </div>
          </div>
